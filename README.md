...
Run only the collectors that apply. GPU is a no-op without `nvidia-smi`. Farm needs a local farmer and full node. Recompute needs `chia_recompute_server` in the journal. Harvester follows `plots were eligible for farming … Time: N s. Total N plots` in `debug.log` (via `chia_root()`). Chia paths are uid 1000's `.chia/mainnet`, never `/root` and never a username. Power supply batteries are any sysfs node with `type=Battery` and a `capacity` file (Logitech `hidpp_battery_*`, laptop `BAT*`, and the like). The collector rescans that class dir each second so plug/unplug is picked up without a restart, and clears retained discovery when a node disappears. `scope=System` (laptop/UPS) stays on the host HA device; other scopes get their own HA device via the host so a mouse battery does not claim the host device battery badge. XMRig needs its HTTP API on localhost (`XMRIG_HOST`/`XMRIG_PORT`/`XMRIG_TOKEN` in `.env`); the mining switch requires `http.restricted=false` (XMRig is read-only or full write — there is no pause-only ACL). Reject ratio is `(shares_total - shares_good) / shares_total`. Profitability always runs. Default spot €/kWh is the public porssisahko API (`api.porssisahko.net/v2/latest-prices.json`, energy-only / no transfer fee, snt/kWh ÷ 100), held HTTPS, RAM-cached, few polls/day (refetch when the current slot is missing or future coverage ≤ 12 h, at most once per 3600 s). Overrides, in order: fixed `ELECTRICITY_EUR_PER_KWH`, MQTT `ELECTRICITY_EUR_PER_KWH_TOPIC`, or `HA_TOKEN` → HA entity `sensor.porssisahko_electricity_price` (`HA_ELECTRICITY_ENTITY` / `HA_URL`, polled every 60 s). It also waits for retained MQTT from package power, `xmr_per_hs_day`, and XMR/EUR before publishing the factor. Factor is `revenue/cost` (1.0 ≈ break-even). Publish-only — no default automation. While paused, last mining hashrate and package watts stay in RAM. `support/` holds helpers that are not host metrics: `xmr_eur_price.py` publishes Kraken’s XMR/EUR last trade; `xmr_network.py` publishes Monero difficulty, network hashrate, last-block reward, and XMR per H/s per day (`reward * 86400 / difficulty`) for hashrate profitability. Expected XMR/day ≈ local hashrate × that rate.
