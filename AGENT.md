...
- `xmrig.py` — held HTTP to local XMRig `/2/summary` + `/json_rpc`; hashrate (60s), reject ratio, mining switch (pause/resume); needs `http.restricted=false` for the switch (XMRig has no pause-only ACL)
- `xmrig.service` — `/root/mqtt-sensors`
