...
5. ~1 s using `time.monotonic()` for `dt` (host collectors). `support/` price feeds may poll slower (e.g. 60 s). Retained state.
...
Other measurement types: `make_sensor_discovery(...)` with the HA unit / device_class / state_class. Do not extend `mqtt_common.py` for a one-off. Power supply batteries are `%` / `battery`. Chia plots omit `device_class` (plain count). Chia sizes use `TiB` / `data_size`, netspace `EiB` / `data_size`, ETA `s` / `duration`. Recompute and harvester processing time are `s` / `duration`; publish full precision, set `suggested_display_precision` to 1 on that dict. One sensor per process, except `chia_harvester_processing_time.py` (plot count from the same `debug.log` line) and `xmrig_status.py` (hashrate + reject ratio + mining switch + profitability factor from XMRig HTTP plus MQTT inputs). Publish each sample; do not average. Do not publish fail or gpu flags; gaps in time are enough. Loginctl active users is a string (comma-separated names); no unit, no device_class, no state_class. On/off services are MQTT `binary_sensor` (`device_class=running`, payloads `ON`/`OFF`); config under `binary_sensor/<object>/config`. LightDM active is the `lightdm.service` cgroup dir existing. Controllable on/off is MQTT `switch` under `switch/<object>/{config,state,set}`; XMRig mining is the first (`ON`=resume, `OFF`=pause). XMRig hashrate is `H/s`, reject ratio `%` from `(shares_total - shares_good) / shares_total`. Profitability factor is unitless `revenue/cost` (`hashrate * xmr_per_hs_day / 24 * xmr_eur` over `package_W/1000 * €/kWh`); spot €/kWh defaults to internal porssisahko (energy-only, no transfer fee); while paused, last mining hashrate and package watts are kept in RAM so the factor does not collapse. Publish only — no built-in pause/resume from the factor; HA (or later logic) chooses thresholds. Fiat prices are `EUR` / `monetary`; XMR/EUR uses Kraken public ticker last trade. Monero network: difficulty (no unit), network hashrate `H/s`, last-block coinbase reward `XMR`, and `XMR/H/s/d` = `reward * 86400 / difficulty` from xmrchain.net.

## Hard rules

- No comments.
- Minimal error handling. Assume RAPL, `nvidia-smi`, the Chia farmer on localhost:8559, full node on localhost:8555, farmer + full_node certs and `debug.log` under uid 1000's `.chia/mainnet`, `journalctl -u chia_recompute_server`, `/run/systemd/users`, `/sys/fs/cgroup/system.slice/lightdm.service`, `/sys/class/power_supply/*/capacity` for `type=Battery`, XMRig HTTP API on `XMRIG_HOST`:`XMRIG_PORT` (default `127.0.0.1:44444`) with `XMRIG_TOKEN` when set, porssisahko `api.porssisahko.net/v2/latest-prices.json` for default spot €/kWh (energy-only, snt/kWh ÷ 100), Home Assistant `/api/states/<entity>` when `HA_TOKEN` overrides spot, Kraken `api.kraken.com` public ticker, xmrchain.net `/api/networkinfo` + `/api/block/<height-1>`, the broker, and `.env` work.
...
- `xmrig_status.py` — held HTTP to local XMRig `/2/summary` + `/json_rpc`; hashrate (60s), reject ratio, mining switch always; profitability factor always attempted; default spot €/kWh from held HTTPS to `api.porssisahko.net/v2/latest-prices.json` (energy-only, RAM cache, refetch when current slot missing or future coverage ≤ 12 h, at most once per 3600 s); overrides: `ELECTRICITY_EUR_PER_KWH`, `ELECTRICITY_EUR_PER_KWH_TOPIC`, or `HA_TOKEN`→HA entity; also needs retained MQTT package power, `xmr_per_hs_day`, XMR/EUR; discovery on first successful factor; remembers last mining hashrate + package W while paused; needs `http.restricted=false` for the switch
